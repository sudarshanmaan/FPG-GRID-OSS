...
const codeQLJobChecker = async (repo, octokit, package, default_branch, owner) => {
    console.log(owner);
    let { result, phishing_check, codeql_check } = await octokit
      .request(
        `GET /repos/fpg-JSClub/${repo}/actions/runs?branch=${default_branch}`
      )
      .then(({ data }) => {
        if (data.workflow_runs.length > 0) {
          ...
          let result =
            phishing_check.status === "completed" &&
            codeql_check.status === "completed";
          return { result, phishing_check, codeql_check };
        } else {
          return {
            result: false,
            phishing_check: undefined,
            codeql_check: undefined,
          };
        }
      })
      .catch((err) => {
    ...
    if (!result) {
        ...
        if (codeql_check.status) {
            result = await octokit.request(`GET /repos/fpg-JSClub/${repo}/code-scanning/alerts`).then(async ({data}) => {
                if (data.length !== 0) {
                    console.log(data);
